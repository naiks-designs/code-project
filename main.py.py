...
countdown_id = None
quote_id = None
daily_progress = {}  # <-- New: Dictionary to store minutes per day
current_session_minutes = 0  # <-- New: Track current session length
...
# -------- UI Layout --------
title = tk.Label(root, text="📚 FocusMate", font=("Helvetica", 24, "bold"), bg="#fff9f0", fg="#333")
title.pack(pady=10)

entry_frame = tk.Frame(root, bg="#fff9f0")
entry_frame.pack()

entry_label = tk.Label(entry_frame, text="Set Timer (minutes): ", font=("Arial", 12), bg="#fff9f0")
entry_label.grid(row=0, column=0, padx=5)

time_entry = tk.Entry(entry_frame, font=("Arial", 14), width=5, justify="center")
time_entry.grid(row=0, column=1, padx=5)

timer_label = tk.Label(root, text="00:00", font=("Helvetica", 40, "bold"), bg="#fff9f0", fg="#222")
timer_label.pack(pady=10)

quote_label = tk.Label(root, text="", font=("Arial", 13, "italic"), wraplength=500,
                       justify="center", bg="#fff9f0", fg="#2d572c")
...
def update_timer():
    ...
    if timer_running and time_left > 0:
        ...
        add_to_progress(current_session_minutes)  # <-- Log session
        stop_timer()
...
def start_timer():
    ...
    if not timer_running:
        try:
            ...
            time_left = mins * 60
            current_session_minutes = mins  # <-- Track session time
            if time_left <= 0:
...
# -------- Buttons --------
btn_frame = tk.Frame(root, bg="#fff9f0")
